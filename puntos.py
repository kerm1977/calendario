...
def comprar_puntos():
    ...
    try:
        # --- CÁLCULO FINANCIERO CORREGIDO (RETENCIÓN 5%) ---
        deduccion = int(cantidad_bruta * 0.05) # 5% para gastos admin de La Tribu
        cantidad_neta = cantidad_bruta - deduccion
        
        # Acreditar solo el NETO al usuario
        member.puntos_totales += cantidad_neta
        
        # Registro transparente en el historial (Mostramos neto y explicamos deducción)
        db.session.add(PointLog(
            member_id=member.id,
        ...
        # NOTIFICACIÓN ADMIN (Detallada con la ganancia)
        db.session.add(AdminNotification(
            category='success',
            title='Ingreso por Compra de Puntos',
            message=f'{member.nombre} compró {cantidad_bruta} pts. Neto acreditado: {cantidad_neta}. Retención: {deduccion} (Gastos Admin).',
            action_link=url_for('puntos.detalle_miembro', member_id=member.id)
        ))
        
        db.session.commit()
        flash(f'Compra exitosa. Se acreditaron {cantidad_neta} puntos (se dedujo el 5% por gastos administrativos).', 'success')
